...
class DatabaseCrawlerSpider(CrawlSpider):
    ...
    start_urls = ['http://https://ngosindia.org//']

    le_main_pages = LinkExtractor(restrict_xpaths="//a[@rel = 'noopener noreferrer']")#main state pages)
    main_pages_rule =  Rule(le_main_pages, follow =  False)

    le_next_page = LinkExtractor(restrict_xpaths="//a[@class = 'lcp_nextlink']/@href")#extracts next page
    next_page_rule = Rule(le_next_page,follow = True)#do this on every page

    le_ngo_details = LinkExtractor(restrict_xpaths="//ul[@class = 'lcp_catlist']/li/a/@href")#extracts ngo links for each page
